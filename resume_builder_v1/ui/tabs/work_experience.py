...
class WorkExperienceUI:
        ...
        if len(local_list_workExps) == 0:
            ...
            for local_obj_workExperience in local_list_workExps:
                ...
                with st.expander(f"{local_obj_workExperience.str_title} @ {local_obj_workExperience.str_companyName}"):
                    ...
                    local_obj_workExperience.dateTime_startDate = st.date_input("Start Date", value=local_dateTime_startDate).strftime("%Y-%m-%d")
                    local_obj_workExperience.str_description = st.text_input("Description of Roles and Responsibilities", value=local_obj_workExperience.str_description)
                    local_obj_workExperience.list_achievements = st.text_area("Top achievements (Seperated by newline)", value="\n".join(local_obj_workExperience.list_achievements)).split("\n")
                    local_obj_workExperience.list_skills = st.text_area("List of skills (Seperated by ',')", value = ",".join(local_obj_workExperience.list_skills)).split(",")
            
            local_button_updateExisting = st.button("Update work expereince")  
    ...
    def add_new_work_experience(self):
        ...
        with st.container(border=True):
            ...
            local_obj_workExperience.str_companyName = st.text_input("Name of the company", value=local_obj_workExperience.str_companyName)
            local_obj_workExperience.str_title = st.text_input("Designation / Role", value=local_obj_workExperience.str_title)
            local_obj_workExperience.dateTime_endDate = st.date_input("End Date", value="today").strftime("%Y-%m-%d")
            local_obj_workExperience.dateTime_startDate = st.date_input("Start Date", value="today").strftime("%Y-%m-%d")
            local_obj_workExperience.str_description = st.text_input("Description of Roles and Responsibilities", value=local_obj_workExperience.str_description)
            local_obj_workExperience.list_achievements = st.text_area("Top achievements (Seperated by newline)", value="\n".join(local_obj_workExperience.list_achievements)).split("\n")
            local_obj_workExperience.list_skills = st.text_area("List of skills (Seperated by ',')", value = ",".join(local_obj_workExperience.list_skills)).split(",")
